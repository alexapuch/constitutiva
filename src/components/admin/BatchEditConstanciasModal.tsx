...
interface BatchEditConstanciasModalProps {
  isOpen: boolean;
  onClose: () => void;
  entries: any[];
  onSuccess: () => void;
...
export default function BatchEditConstanciasModal({ isOpen, onClose, entries, onSuccess }: BatchEditConstanciasModalProps) {
  ...
  useEffect(() => {
    if (isOpen && entries.length > 0) {
      // Tomamos los datos de la primera constancia como base
      setCommercialName(entries[0].commercial_name || '');
      setAddress(entries[0].address || '');
  ...
  const handleSave = async () => {
    if (!commercialName.trim()) {
      Swal.fire({ icon: 'error', title: 'Error', text: 'El nombre de la empresa es requerido.' });
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/constancias/batch`, {
        method: 'PUT',
      ...
      if (res.ok) {
        Swal.fire({ icon: 'success', title: 'Guardado', text: `Se actualizaron ${folios.length} constancias.`, timer: 1500, showConfirmButton: false });
        onSuccess();
        onClose();
      } else {
      ...
      Swal.fire({ icon: 'error', title: 'Error', text: 'No se pudieron actualizar las constancias.' });
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            <div className="bg-blue-900 p-5 text-white flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-xl font-bold">Editar Constancias en Lote</h3>
                <p className="text-sm text-blue-200 mt-1">{entries.length} constancias seleccionadas</p>
              </div>
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-xl text-sm text-blue-800 dark:text-blue-200 mb-2">
                Estos cambios se aplicarán a <strong>todas las constancias</strong> de este grupo. El nombre del empleado permanecerá intacto para cada uno.
              </div>
              <div>
